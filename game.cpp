...
Game::Game() {
  for (int i = 0; i < 10; i++)
    for (int j = 0; j < 10; j++) this->addItem(board[i][j] = new Square(i, j));
  newGame(Color::WHITE);
}
  ...
  for (int i = 0; i < 10; i++)
    for (int j = i % 2 ^ 1; j < 10; j += 2)
      if (conn->getplayerPlace() != 0 &&
          board[i][j]->getPiece()->getColor() == turn)
...
Color Game::getTurn() { return turn; }

Game::Proxy Game::operator[](int index) { return Proxy(board[index]); }
...
void Game::setConnection(Connection *conn) { this->conn = conn; }

Connection *Game::getConnection() { return conn; }
...
int Game::moveLength() {
  ...
  for (int i = 0; i < 20; i++) moves[i].clear();
  for (int i = 0; i < 10; i++)
    for (int j = i % 2 ^ 1; j < 10; j += 2)
...
std::vector<std::vector<std::tuple<int, int, int>>> Game::possibleMoves(
    int length) {
  ...
  for (int i = length - 1; i > 0; i--)
    for (auto j : v[i])
      for (auto k : moves[i - 1])
        if (std::get<0>(j) == std::get<1>(k)) v[i - 1].push_back(k);
  return v;
}
...
Game::Proxy::Proxy(Square **array) : array(array) {}

Square *Game::Proxy::operator[](int index) { return array[index]; }
