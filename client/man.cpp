...
Man::Man(Color color) : Piece(color) {}
  ...
  if (x + k * a >= 0 && x + k * a < 10 && y + k * b >= 0 && y + k * b < 10 &&
    ...
    g[game[0][x][y]->getIndex()].push_back(
        {game[0][x + k * a][y + k * b]->getIndex(),
         game[0][x + a][y + b]->getIndex()});
    g[game[0][x + k * a][y + k * b]->getIndex()].push_back(
        {game[0][x][y]->getIndex(), game[0][x + a][y + b]->getIndex()});
  }
}
...
void Man::addMove(std::vector<std::pair<int, int>> &v, int x, int y, int a,
                  int b) {
