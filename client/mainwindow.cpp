...
Game *MainWindow::getGame() { return game; }
...
void MainWindow::enableDraw(bool enable) { ui->drawButton->setEnabled(enable); }
...
void MainWindow::on_place1Button_clicked() {
  if (game->getConnection()->getplayerPlace() == 0) {
    game->getConnection()->sendMessage("sit1 " +
                                       game->getConnection()->getNick());
...
void MainWindow::on_place2Button_clicked() {
  if (game->getConnection()->getplayerPlace() == 0) {
    game->getConnection()->sendMessage("sit2 " +
                                       game->getConnection()->getNick());
...
void MainWindow::on_drawNo_clicked() { ui->drawOffer->hide(); }
